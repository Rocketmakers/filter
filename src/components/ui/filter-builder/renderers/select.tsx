...
export const FilterSelectRenderer = ({
  ...
  const addOrUpdateFilter = useCallback(
    (config: FilterConfig, option: FilterBaseOption) => {
      ...
      if (isAlreadySelected) {
        // Deselecting in a single-pick filter (or removing the last value)
        // removes the pill entirely — unless locked.
  ...
  const initialSelectedOptions = useMemo(
    () => currentFilter?.value ?? [],
    // intentionally captured once when the renderer mounts
