...
const resolveDefaultScopeChoice = (scopeChoices, suggestedScope) => {
  if (!suggestedScope) {
    return NO_SCOPE;
  }
  ...
  const { customScope } = await cz.prompt([
    {
      ...
      default:
        suggestedScope && !scopeChoices.includes(suggestedScope)
          ? suggestedScope
          : undefined,
...
const manualCommit = async (cz, commit, defaults) => {
...
const printCommitPreview = (commitParts) => {
  const preview = finalizeCommitParts(commitParts);

  console.log("\n  ---------------------------------------------");
  console.log(
    `  ${preview.type}${formatScope(preview.scope)}: ${preview.subject}`,
  );
...
const promptForCommit = async (cz, commit, aiSuggestion) => {
  const defaultTypeIndex = aiSuggestion
    ? Math.max(
        TYPES.findIndex((type) => type.value === aiSuggestion.type),
        0,
      )
    : 0;
